...
static ssize_t dev_read(struct file *file, char __user *buffer, size_t len, loff_t *offset) {
    ...
    *offset += copy_len;
    printk(
        KERN_INFO "Phonebook: successfully copied the message (%d chars) to user space, new offset: %lld\n",
        copy_len,
        *offset
    );
    return copy_len;
...
static ssize_t dev_write(struct file *file, const char __user *buffer, size_t len, loff_t *offset) {
    ...
    int copy_len = min(MSG_BUFFER_SIZE, len);
    if (copy_len <= 0)
        return 0;

    error_count = copy_from_user(message_buffer, buffer, copy_len);
    if (error_count != 0) {
        printk(KERN_ERR "Phonebook: failed to copy %d bytes from the user space\n", error_count);
        return -EFAULT;
    }

    message_size = copy_len;
